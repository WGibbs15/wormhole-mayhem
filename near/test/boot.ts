...
const tokenMethods = {
  viewMethods: [],
  changeMethods: ["boot_token_bridge", "submit_vaa", "submit_vaa_callback"],
};
...
async function initTest() {
  ...
  const tokenContract = await fs.readFile("./contracts/token_bridge/target/wasm32-unknown-unknown/release/token_bridge.wasm");
...
async function test() {
    ...
    await tokenUseContract.boot_token_bridge({ args: { core: config.wormholeAccount } });
